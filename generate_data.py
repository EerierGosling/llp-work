...
class GaussianDiffusion:
    ...
    def sample_from_reverse_process(
        self, model, xT, timesteps=None, model_kwargs={}, ddim=False, guidance_scale=2.0
    ):
        ...
        for i, t in zip(np.arange(timesteps)[::-1], new_timesteps[::-1]):
            with torch.no_grad():
                ...
                if guidance_scale > 1.0:
                    ...
                    stored_cond_gradients.append(cond_score.detach().cpu())
                    stored_inputs.append(final.detach().cpu())
                    # Combine predictions using guidance_scale
                    pred_epsilon = unconditional_pred + guidance_scale * (conditional_pred - unconditional_pred)
                else:
                ...
                final = final.detach()
        return final, torch.stack(stored_cond_gradients).permute(1, 0, 2, 3, 4), torch.stack(stored_inputs).permute(1, 0, 2, 3, 4)
...
def sample_N_images(
    ...
    Args:
        ...
        end_idx : Ending index for sampling (exclusive)
        model : Diffusion model
    ...
    for batch_idx in tqdm(range(total_batches)):
        ...
        current_batch_size = min(batch_size, N - num_samples)
        
        xT = (
            torch.randn(current_batch_size, num_channels, image_size, image_size)
            .float()
            .to(args.device)
        )

        # Calculate actual indices for this batch
        current_start = start_idx + batch_idx * batch_size
        y = torch.arange(current_start, current_start + current_batch_size) % num_classes
        y = y.to(args.device) + 1  # Adding 1 to match the existing code
        
        gen_images, cond_gradients, input = diffusion.sample_from_reverse_process(
            model, xT, sampling_steps, {"y": y}, args.ddim, args.guidance_scale
        )
        
        # Store scores
        all_cond_gradients.append(cond_gradients.cpu().numpy())
        ...
        if batch_idx % 10 == 0:
            print(f"Sampling progress: {num_samples}/{N} images ({current_start} to {current_start + current_batch_size - 1})")
            
    samples = np.concatenate(samples).transpose(0, 2, 3, 1)[:N]
    samples = (127.5 * (samples + 1)).astype(np.uint8)
    return (samples, np.concatenate(labels), np.vstack(all_cond_gradients), np.vstack(all_inputs))
...
def main():
    ...
    parser.add_argument("--delete-keys", nargs="+", help="Keys to delete from pretrained ckpt")
    parser.add_argument(
        "--start-idx",
        type=int,
        default=0,
        help="Starting index for sampling images",
    )
    parser.add_argument(
        "--end-idx",
        type=int,
        default=50000,
        help="Ending index for sampling images (exclusive)",
    )
    # misc
    ...
    print("We are assuming that model input/output pixel range is [-1, 1]. Please adhere to it.")
    ...
    # Sampling
    model.eval()
    sampled_images, labels, cond_gradients, inputs_list = sample_N_images(
        args.start_idx,
        args.end_idx,
        model,
        diffusion,
    ...
    output_filename = os.path.join(
        args.save_dir,
        f"{args.arch}_{args.dataset}-{args.sampling_steps}-sampling_steps-{args.start_idx}_to_{args.end_idx-1}-guidance_{args.guidance_scale}.npz",
    )
